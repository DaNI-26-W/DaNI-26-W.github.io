...
## Import data
cantril <-
    read.csv(here("posts/welcome/Data/happiness-cantril-ladder.csv")) %>%
...
cantril %>%
  ...
  mutate(Relative_change = round((Absolute_change/`2011`), 3)) %>% 
...
p <- ggplot(
  cantril_data,
  aes(x = Year, y = `Cantril.ladder.score`, colour = Country)
) +
...
cantril_2023 <- cantril %>% 
  filter(Year == 2023) %>% 
  select(Country, `Cantril.ladder.score`)
...
top_bottom_10 <- cantril_2023 %>% 
  ...
  mutate(
    Country = reorder(Country, `Cantril.ladder.score`)
  )

ggplot(
  top_bottom_10,
  aes(x = `Cantril.ladder.score`, y = Country,
      fill = `Cantril.ladder.score`)
) +
  geom_col() +
  labs(
